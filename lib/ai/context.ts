 ...
 * collapse into one group (`82.5x8,8,7` = three sets at 82.5 kg). Mixed weights
 * are space-separated groups (`80x8 82.5x8 85x6`). `@` carries RPE, `bw` means
 * bodyweight, and `eNNN` is the estimated 1RM for that session — precomputed
 * because arithmetic is exactly what a language model should not be doing.
 *
...
function serialiseSets(sets: SetLine[]): string {
    ...
    return groups
        .map((group) => {
            const weight = group.weight === 0 ? 'bw' : formatWeight(group.weight);
