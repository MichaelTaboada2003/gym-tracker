...
import { analyse, AnalysisError, isConfigured } from '../lib/ai/client';
import { Analysis, parseAnalysis } from '../lib/ai/prompt';
...
export interface AnalysisState {
    ...
    /** False when this build was compiled without an analysis endpoint. */
    configured: boolean;
...
export function useWorkoutAnalysis(sessionId: string | null) {
    ...
    const generate = useCallback(async () => {
        ...
        try {
            ...
            const result = await analyse(context.text, controller.signal);
